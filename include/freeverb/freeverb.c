...
void fv_init(fv_Context *pCtx, f32 pWet, f32 pRoomSize, f32 pDry, f32 pDamp, f32 pWidth) {
    ...
    {
        pCtx->allpassl[i].feedback = 0.5;
        pCtx->allpassr[i].feedback = 0.5;
    }

    fv_set_param(pCtx, pWet, pRoomSize, pDry, pDamp, pWidth);
}
...
void fv_set_param(fv_Context *ctx, f32 pWet, f32 pRoomSize, f32 pDry, f32 pDamp, f32 pWidth) {
    ...
    fv_set_wet(ctx, pWet);
    fv_set_roomsize(ctx, pRoomSize);
    fv_set_dry(ctx, pDry);
    fv_set_damp(ctx, pDamp);
    fv_set_width(ctx, pWidth);
}
...
void fv_mute(fv_Context *ctx) {
    ...
    {
        zmem(ctx->combl[i].buf, sizeof(ctx->combl[i].buf));
        zmem(ctx->combr[i].buf, sizeof(ctx->combr[i].buf));
    }
    for (int i = 0; i < FV_NUMALLPASSES; i++)
    {
        zmem(ctx->allpassl[i].buf, sizeof(ctx->allpassl[i].buf));
        zmem(ctx->allpassr[i].buf, sizeof(ctx->allpassr[i].buf));
    }
}
